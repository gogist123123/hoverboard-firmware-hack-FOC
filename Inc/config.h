...
//play melody after turn on
#define POWER_ON_PLAY_MELODY
...
#define CONTROL_SERIAL_KCQ
  ...
  // #define DUAL_INPUTS                 //  UART*(Primary) + SIDEBOARD(Auxiliary). Uncomment this to use Dual-inputs
  #define PRI_INPUT1             1, 650, 0, 2600, 30     // TYPE, MIN, MID, MAX, DEADBAND. See INPUT FORMAT section
  #define PRI_INPUT2             1, 690, 0, 2800, 30    // TYPE, MIN, MID, MAX, DEADBAND. See INPUT FORMAT section
  #ifdef DUAL_INPUTS
    #define FLASH_WRITE_KEY      0x1102  // Flash memory writing key. Change this key to ignore the input calibrations from the flash memory and use the ones in config.h
    // #define SIDEBOARD_SERIAL_USART2 1   // left sideboard
    //#define SIDEBOARD_SERIAL_USART3 1   // right sideboard
  ...
  // #define SUPPORT_BUTTONS_RIGHT      // use right sensor board cable for button inputs. Disable DEBUG_SERIAL_USART3!
#endif
// ######################## END OF VARIANT_KCQ_DASH SETTINGS #########################
...
// ########################### UART SETIINGS ############################
  ...
  #define SERIAL_START_FRAME      0xa5                 // [-] Start frame definition for serial commands
...
// ############################### VALIDATE SETTINGS ###############################
#if !defined(VARIANT_KCQ_DASH) && !defined(VARIANT_ADC) && !defined(VARIANT_USART) && !defined(VARIANT_NUNCHUK) && !defined(VARIANT_PPM) && !defined(VARIANT_PWM) && \
